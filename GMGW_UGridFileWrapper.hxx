...
public:
  ...
  virtual void
  getNextCellConnectivity(GMGW_int& nConn, GMGW_int connect[]) const;
  virtual void
  getNextVertexCoords(double& x, double& y, double &z) const;
...
private:
  ...
  int64_t
  convertToInt64(const unsigned char raw[4]) const;
  double
  convertToDouble(const unsigned char raw[8]) const;
};
